...
class ShortSerializer: Serializer<Short>() {
    override fun write(configuration: Configuration, output: Output, serializableValue: Short) =
        output.writeShort(serializableValue)

    override fun read(configuration: Configuration, input: Input, type: KClass<out Any>): Short =
        input.readShort()
}