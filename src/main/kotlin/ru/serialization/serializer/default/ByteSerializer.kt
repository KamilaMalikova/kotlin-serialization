...
class ByteSerializer: Serializer<Byte>() {
    override fun write(configuration: Configuration, output: Output, serializableValue: Byte) =
        output.writeByte(serializableValue)

    override fun read(configuration: Configuration, input: Input, type: KClass<out Any>): Byte =
        input.readByte()
}